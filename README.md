...
- Also watched: a five-minute Lambda asks whether the host should be up and whether it is, and `platform-ops-prod-host-not-running` mails after fifteen minutes of "yes and no". That is the case the alarm above cannot reach, because CloudWatch mails on a change of state and a host that broke while muted, or never started, never changes. Its alarm treats missing data as breaching, so the poller going quiet mails too.

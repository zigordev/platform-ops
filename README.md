...
- Watched: the CloudWatch alarm `platform-ops-prod-host-status` mails within minutes when the host stops or goes impaired, deliberately or not. It runs in AWS rather than on the host, which is the only way to be told the host is gone. Set `host_alarm_email` and click the link AWS sends, or the subscription stays pending and the alarm mails nobody.

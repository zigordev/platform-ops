...
export const sloOverview: DashboardSpec = {
  ...
  description: 'The four objectives the alerts are built on: each indicator over its alerting windows, and how fast its error budget is being spent.',
  folder: ESTATE,
  tags: ['slo'],
  ...
  rows: [
    [
      ...
      stat('Page latency · 6 hours', 'cv page renders under 512 ms over the last six hours. The objective is 95%.', { w: 6, h: 5 }, [
        prom('slo:page_latency:ratio_rate6h', { legend: 'cv-web' }),
      ], { unit: 'percentunit', decimals: 2, steps: atLeast(0.95), text: 'value_and_name' }),
    ...
    [
      timeseries('Page latency · cv', 'Share of cv page renders under 512 ms, from Tempo span metrics, over one and six hours.', { w: 12, h: 8 }, [
      ...
      timeseries('Page latency budget burn · 1 hour', 'Spend against the 5% page latency budget. Both page latency alerts open tickets.', { w: 12, h: 8 }, [
        prom('(1 - slo:page_latency:ratio_rate1h) / 0.05', { legend: 'cv-web' }),
      ], { unit: 'short', min: 0, steps: BURN, lines: true }),
    ],
    [
    ...
    [
      timeseries('Traffic gates', 'Below these the alerts stay silent: about 3 API requests a minute, 20 page views an hour, and any email in the hour.', { w: 24, h: 7 }, [
        ...
        prom('slo:page_traffic:rate1h * 3600', { legend: 'cv-web · page views an hour' }),

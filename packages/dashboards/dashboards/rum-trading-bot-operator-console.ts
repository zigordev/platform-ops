...
export const rumTradingBotOperatorConsole: DashboardSpec = {
  ...
  description:
    'The trading-bot console in the browser it is actually driven from: the vitals per page, what was clicked, what broke and what the content security policy caught. The console is an internal tool with one or two operators, so read counts here as "did this person have a bad time", not as traffic. trading-bot is local only, so nothing on this dashboard has a production counterpart. CSP violations and the browser log panels arrive with trading-bot#158; the vitals, navigations and errors are live now.',
  folder: FRONTEND,

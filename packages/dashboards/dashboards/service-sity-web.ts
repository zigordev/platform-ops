...
export const serviceSityWeb: DashboardSpec = {
  ...
  title: 'sity-web',
  description:
    'The server that hands out the sity scene, and the browser that draws it: requests, health, Core Web Vitals, what broke in the browser and what the content security policy caught. sity runs on a laptop and nowhere else — it has no deploy, no production compose and no production scrape — so there is no objective here to breach and no alert to page on. Two panel families are missing on purpose: sity serves files and calls nothing, so it ships no OpenTelemetry SDK and has no traces, and its /health reports no components, so there is no dependency table. Release reads dev and no deploy is ever marked, because sity’s compose sets no release for the server to report; fix that in sity and both start working. It stays empty until sity#36 ships the instrumentation and platform-ops#205 adds the local scrape job for sity-web.',
  folder: SERVICES,
  tags: ['service', 'sity'],

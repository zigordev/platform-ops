...
const SEL = 'job="cv-web"';
const FUNNEL = 'ask-opened|ask-submitted|ask-answer-shown|ask-failed|contact-opened|contact-sent|contact-failed|cv-downloaded|case-study-opened|locale-switched|render-error';
...
export const rumCv: DashboardSpec = {
  uid: 'rum-cv',
  title: 'RUM · cv',
  description: 'cv.zigordev.com as visitors experience it: the vitals by release, what they did, what broke in their browser and what the content security policy caught.',
  folder: FRONTEND,
  tags: ['rum', 'cv'],
  ...
  rows: [
    ...
    [
      table('What visitors did', 'The site’s named events over the time range, most frequent first.', { w: 8, h: 9 }, [
    ...
    [
      logs('Browser errors', 'Each distinct error once per ten minutes, with the source line it maps to.', { w: 8, h: 10 }, [loki('{app="cv-web"} | json | event="rum.client_error"')]),
      logs('Poor vitals', 'Visits with a poor vital, and the element behind it.', { w: 8, h: 10 }, [loki('{app="cv-web"} | json | event="rum.vital_poor"')]),
      logs('CSP violations', 'What was blocked, where it came from and on which page.', { w: 8, h: 10 }, [loki('{app="cv-web"} | json | event="csp.violation"')]),
    ],
  ],
};
